...
public class No implements Position{
	...
	public void calcula_retangulo_minimo(No elemento, boolean rotacao){
		...
		if(elemento.getItem() == null){
			...
			if( tipo.getTipoCorte() == "horizontal"){
			...
			System.out.println("\nVou calcular o retângulo Mínimo de um nó item");
				...
				retmi.setH_p(item.getAltura());
				retmi.setW_p(item.getLargura());
				
				elemento.setRetanguloMinimo(retmi);
				
				if(rotacao == true){
				
					retmi.setH_p(item.getLargura());
					retmi.setW_p(item.getAltura());
	...
	public void calcula_retangulo_maximo(No elemento, boolean rotacao){
		...
		pai = elemento.getPai();
			...
			if(pai == null){
				...
				retMax.setW_P(Chapa.getLargura());
				retMax.setH_P(Chapa.getAltura());
